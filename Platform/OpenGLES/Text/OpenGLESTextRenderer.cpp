#include "OpenGLESTextRenderer.h"
...
namespace MPE
{
OpenGLESTextRenderer::OpenGLESTextRenderer(const std::string& shaderPath, const std::string& fontPath, int fontSize, OrthographicCamera* camera)
{
...
OpenGLESTextRenderer::~OpenGLESTextRenderer()
{
    FT_Done_Face(m_Face);
    FT_Done_FreeType(m_FT);
}

void OpenGLESTextRenderer::OnEvent(Event& e)
{
    EventDispatcher dispatcher(e);
    dispatcher.Dispatch<WindowResizeEvent>(MPE_BIND_EVENT_FUNCTION(OpenGLESTextRenderer::OnWindowResize));
}

void OpenGLESTextRenderer::InitializeFont()
{
    // Initialize FreeType
...
void OpenGLESTextRenderer::UnInitializeFont()
{
    m_shader->Unbind();
    FT_Done_Face(m_Face);
    FT_Done_FreeType(m_FT);
...
void OpenGLESTextRenderer::UpdateProjection(float screenWidth, float screenHeight)
{
    // GLint vp[4];
    ...
    m_shader->Bind();
    std::dynamic_pointer_cast<MPE::OpenGLESShader>(m_shader)->InjectUniformMat4("UNI_PROJECTION_MATRIX", projection);
}

void OpenGLESTextRenderer::LoadCharacters()
{
...
void OpenGLESTextRenderer::RenderText(const std::string& text, float x, float y, float scale, const glm::vec4& color)
{
    m_shader->Bind();
    std::dynamic_pointer_cast<MPE::OpenGLESShader>(m_shader)->InjectUniformFloat4("UNI_TEXT_COLOR", color);
...
void OpenGLESTextRenderer::SetFontSize(float fontSize)
{
...
bool OpenGLESTextRenderer::OnWindowResize(WindowResizeEvent& e)
{
