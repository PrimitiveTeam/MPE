...
void Renderer::SubmitLines(const REF<Shader> &shader, const REF<VertexArray> &vertexArray, const glm::mat4 &transform)
{
    shader->Bind();
    ...
    vertexArray->Bind();
    RenderPrimitive::DrawLines(vertexArray, vertexArray->GetIndexBuffer()->GetCount());
}
}